...
def _apply_locked(audit_root: Path, delta: Mapping[str, Any], *,
    ...
    # Decision provenance (spec §5, simplified in v2.x): the delta itself is
    # the decision. The runtime records one provenance event per transaction
    # from the top-level ``agent_id`` and ``phase`` (both required at the
    # schema layer), so a separate ``decisions[]`` array is no longer carried
    # on the wire or in the ledger. A future audit reads ``derived_events``
    # to see who touched canonical state and when.
    txn.derived_events.append({
        "event": "delta_applied",
        "delta_generation": None,  # stamped at commit time below
        "agent_id": str(delta.get("agent_id") or ""),
        "phase": str(delta.get("phase") or ""),
        "ops": sorted(k for k, v in (delta or {}).items()
    ...
    # Stamp the transaction's generation onto the provenance event recorded
    # above, so a future audit can correlate the entry with the exact
    # transaction that committed it. Sync the in-memory derived events
    # onto the ledger dict *before* the persistence write — the prior
    # post-write sync was dead code.
    for event in txn.derived_events:
        if event.get("event") == "delta_applied" and event.get("delta_generation") is None:
            event["delta_generation"] = generation
...
def _apply_assumption_side_effects(txn: _Transaction,
    ...
    for assumption in iter_objects(txn.ledger, "assumption"):
        ...
        # canonical objects. The txn -> ledger sync happens at commit time
        # in apply_delta (one place, not at every event-emit site).
...
#: Removed in Skill-First Refactor v2.x (spec §5 simplified): the
#: ``decisions[]`` array was removed from the schema and the ledger; the
#: provenance of a mutation is now inferred from the delta's top-level
#: ``agent_id`` / ``phase`` and recorded as a derived event on apply.
