...
<!-- Used in phase(s): P16 deep patch-bypass check -->

# patch-bypass-checker (mini-audit inline role)
...
Write your per-patch bypass assessment to `mini-audit/bypass-analysis/<advisory-id>-bypass.md`. The orchestrator will merge these into the KB `## Bypass Analysis` section after all agents complete.
...
Create `mini-audit/bypass-analysis/` directory if it does not exist.
