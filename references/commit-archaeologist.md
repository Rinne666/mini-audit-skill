...
<!-- Used in phase(s): L1 commit archaeology;P1 commit archaeology -->

# commit-archaeologist (mini-audit inline role)
...
```bash
# Commit-count cap (env override: MINI_AUDIT_COMMIT_SCAN_LIMIT)
MAX_COMMITS="${MINI_AUDIT_COMMIT_SCAN_LIMIT:-500}"

# Age cap (env override: MINI_AUDIT_COMMIT_SCAN_SINCE — any value git --since accepts)
MAX_AGE="${MINI_AUDIT_COMMIT_SCAN_SINCE:-60 days ago}"
...
- Standard paths: `auth/`, `mini-audit/`, `crypto/`, `validation/`, `sanitize/`, `middleware/`, `permission/`, `access/`, `login/`, `session/`, `token/`
...
3. Dedup with advisory-hunter: if `mini-audit/attack-surface/knowledge-base-report.md` `## Advisory Intelligence` section exists, extract SHAs already recorded there and remove them
...
### File 1: `mini-audit/attack-surface/commit-recon-report.md`
...
**Scan depth**: up to `$MAX_COMMITS` commits within `$MAX_AGE` across all refs (env: `MINI_AUDIT_COMMIT_SCAN_LIMIT=<N>`, `MINI_AUDIT_COMMIT_SCAN_SINCE=<duration>`)
...
### File 2: Append `## Commit Archaeology` to `mini-audit/attack-surface/knowledge-base-report.md`

Include:
- Priority Commits table (top 30)
- Project security vocabulary discovered (for Phase 3 KB builder context)
- Cross-reference: `mini-audit/attack-surface/commit-recon-report.md`
...
If `mini-audit/attack-surface/knowledge-base-report.md` does not exist, create it. If it exists, append the section.
