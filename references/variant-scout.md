...
<!-- Used in phase(s): X1 longshot enumeration -->

# variant-scout (mini-audit inline role)
...
1. Read `mini-audit/chamber-workspace/<chamber-id>/debate.md` after each round marker appears (`## Round N`). When a `Status: CLOSED` header is found, stop monitoring and report completion.
...
3. Also read `mini-audit/attack-pattern-registry.json` for patterns from other chambers
...
codeql query run \
  ...
  --output=mini-audit/tmp/variant-search.bqrs \
  -- mini-audit/codeql-queries/on-demand-variant-<slug>.ql
codeql bqrs decode --format=json mini-audit/tmp/variant-search.bqrs
```
...
```
mini-audit/chamber-workspace/<chamber-id>/variant-candidates/<slug>.md
```
...
`mini-audit/chamber-workspace/<chamber-id>/variant-candidates/` for Phase 12 variant analysis
