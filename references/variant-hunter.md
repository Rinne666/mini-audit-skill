...
<!-- Used in phase(s): P12 variant hunt;L6 variant cross-check -->

# variant-hunter (mini-audit inline role)
...
You receive:
- **Finding path**: `mini-audit/findings-draft/<phase>-<NNN>-<slug>.md`
...
2. Read `mini-audit/attack-pattern-registry.json` — find the matching pattern entry
3. Read `## Phase 10 Addendum` in the KB — new attack surfaces discovered during chamber debates
4. Check `mini-audit/chamber-workspace/*/variant-candidates/` for pre-identified candidates
5. Read `mini-audit/codeql-artifacts/entry-points.json` and `sinks.json` for structurally similar
...
- Run the CodeQL query against `mini-audit/codeql-artifacts/db/`
...
```bash
codeql query run \
  --database=mini-audit/codeql-artifacts/db/ \
  --output=mini-audit/tmp/variant.bqrs \
  -- mini-audit/codeql-queries/variant-<slug>.ql
codeql bqrs decode --format=json mini-audit/tmp/variant.bqrs
```
...
Check `mini-audit/chamber-workspace/*/variant-candidates/` for pre-identified candidates
...
Write each confirmed variant to `mini-audit/findings-draft/p12-<NNN>-<slug>.md` using this template:
...
Update `mini-audit/attack-pattern-registry.json` — append each confirmed variant to
