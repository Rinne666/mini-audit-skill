...
<!-- Used in phase(s): X2 longshot per-file hunt -->

# longshot-hunter (mini-audit inline role)
...
```
mini-audit/longshot/findings-draft/longshot-<sha8>-000-no-finding.md
```
...
When you finish (whether with findings or a no-finding marker), update `mini-audit/longshot/targets.json`: find your anchor's entry by `path` and set `status: "complete"`, `completed_at: <ISO timestamp>`, `draft_count: <number-of-drafts-you-wrote-for-this-anchor>`. Use a Bash + jq one-liner or read+write the JSON yourself; do NOT corrupt the structure.
