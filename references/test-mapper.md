...
<!-- Used in phase(s): L6 test-coverage mapping;P8 test coverage -->

# test-mapper (mini-audit inline role)
...
- **Test strategies**: `mini-audit/confirm-workspace/env-strategies.json` (test framework info from env-detective)
- **Connection details (optional)**: `mini-audit/confirm-workspace/env-connection.json` — read `test_identities[]` for any auth context the test needs
...
- **Session UUID**: `$MINI_AUDIT_SESSION_UUID` (informational; goes into test name annotation)
...
Read `mini-audit/findings/<ID>-<slug>/report.md`. Extract:
...
```python
import json, os
def mini_audit_token(label="user"):
    with open(os.environ["MINI_AUDIT_CONNECTION"], "r") as f:
...
When invoking the test (Section 6), export `MINI_AUDIT_CONNECTION=mini-audit/confirm-workspace/env-connection.json` so the helper can find it.
...
**Output location**: `mini-audit/findings/<ID>-<slug>/confirm-test.{py|js|go|rb|java|rs|php}`
...
cd <target_dir> && \
  MINI_AUDIT_CONNECTION=mini-audit/confirm-workspace/env-connection.json \
  timeout 90 python -m pytest mini-audit/findings/<ID>-<slug>/confirm-test.py -v --timeout=60 \
  2>&1 | tee mini-audit/findings/<ID>-<slug>/confirm-test-output.log

# JavaScript / Jest
cd <target_dir> && \
  MINI_AUDIT_CONNECTION=mini-audit/confirm-workspace/env-connection.json \
  timeout 90 npx jest mini-audit/findings/<ID>-<slug>/confirm-test.js --no-coverage --testTimeout=60000 \
  2>&1 | tee mini-audit/findings/<ID>-<slug>/confirm-test-output.log
...
cd <target_dir> && \
  MINI_AUDIT_CONNECTION=mini-audit/confirm-workspace/env-connection.json \
  timeout 90 bundle exec rspec mini-audit/findings/<ID>-<slug>/confirm-test_spec.rb --order defined \
  2>&1 | tee mini-audit/findings/<ID>-<slug>/confirm-test-output.log
```
...
**Test naming convention**: include both the finding slug AND the first 8 chars of `$MINI_AUDIT_SESSION_UUID` (`test_confirm_<slug>_<sessionShortID>`) so concurrent confirm runs against the same checkout don't collide on test selectors.
...
Confirm-Test: mini-audit/findings/<ID>-<slug>/confirm-test.{ext}
Confirm-Test-Output: mini-audit/findings/<ID>-<slug>/confirm-test-output.log
