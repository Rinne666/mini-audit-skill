...
<!-- Used in phase(s): L1 intent corpus;P2 deep intent corpus;R0 revisit intent -->

# intent-cartographer (mini-audit inline role)
...
- **Output path**: where to write the corpus JSON (`mini-audit/confirm-workspace/intent-corpus.json` or `mini-audit/attack-surface/intent-corpus.json` depending on mode).
- **Findings inventory path** (optional): `mini-audit/confirm-workspace/findings-inventory.json`. If present, you also run a **cross-check pass** (see Step 4) and write per-finding verdicts.
...
Skip generated, vendored, and lockfile directories: `node_modules/`, `vendor/`, `.git/`, `dist/`, `build/`, `target/`, `mini-audit/` itself.
