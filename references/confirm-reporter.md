...
<!-- Used in phase(s): V1-V7 confirm pass reporting;V6 final confirm report -->

# confirm-reporter (mini-audit inline role)
...
You receive:
- **Findings directory**: `mini-audit/findings/`
- **Confirm workspace**: `mini-audit/confirm-workspace/`
- **Audit state**: `mini-audit/audit-state.json` (optional supplemental metadata only)
- **Intent corpus** (optional): `mini-audit/confirm-workspace/intent-corpus.json` — present if V1.5 Intent Cross-Check completed.
- **Intent verdicts** (optional): `mini-audit/confirm-workspace/intent-verdicts.json` — per-finding `match: yes|partial|no|contested` verdicts. May be absent if V1.5 was skipped or failed.

## Report Protocol

### 1. Inventory All Findings

Read `mini-audit/confirm-workspace/findings-inventory.json` (written by V1) as the source of truth for confirmation scope — it lists every finding, its `dir`, `source_kind`, and `poc_kind`, including theoretical / repaired ones. Then read each finding's `report.md` for the per-finding details.
...
Before writing the report, mirror every finding that received a verdict into `mini-audit/confirm-workspace/confirmed-findings/`, grouped by category. This gives reviewers a single place to scan only the findings the confirmer reached a conclusion on, without having to cross-reference `confirmation-report.md` against `mini-audit/findings/`.

Included categories: `confirmed-live`, `confirmed-test`, `analytical-only`, `confirmed-fp`. Findings in `unconfirmed | inconclusive | blocked | no-poc | error` are NOT staged — they remain only in `mini-audit/findings/` and the report.

```bash
# Wipe any prior staging so the folder reflects only this run.
rm -rf mini-audit/confirm-workspace/confirmed-findings
mkdir -p mini-audit/confirm-workspace/confirmed-findings/{confirmed-live,confirmed-test,analytical-only,confirmed-fp}
```

For each finding whose resolved category is one of the four above:

```bash
cp -R "mini-audit/findings/<ID>-<slug>/" "mini-audit/confirm-workspace/confirmed-findings/<category>/"
```
...
Write `mini-audit/confirmation-report.md`:
...
| Confirmed-findings staging | `mini-audit/confirm-workspace/confirmed-findings/` (grouped by verdict) |
...
(read from `mini-audit/confirm-workspace/findings-inventory.json:by_class`)
...
(read each finding's `poc_kind` from `mini-audit/confirm-workspace/findings-inventory.json`. `runnable` = a real `poc.*`/`exploit.*` script existed at audit time; `theoretical` = only a `poc.theoretical.md` note existed; `none` = no PoC artifact at all. The latter two enter confirmation without a runnable PoC and reach a verdict only via the V5 generated-test fallback.)
...
- **Evidence**: `mini-audit/findings/<ID>-<slug>/confirm-evidence/`
...
- **Test file**: `mini-audit/findings/<ID>-<slug>/confirm-test.{ext}`
- **Test output**: `mini-audit/findings/<ID>-<slug>/confirm-test-output.log`
...
- **Session UUID**: <MINI_AUDIT_SESSION_UUID>
- **Provisioning method**: <method_used>
- **Actual port** (after fallback): <port>
- **Startup duration**: <seconds>
- **Healthcheck**: <endpoint and result>
- **Containers/processes**: <list, all stamped with mini-audit.session=<UUID>>
- **Setup log**: `mini-audit/confirm-workspace/setup.log`
- **Healthcheck-failure log** (only when V3 failed): `mini-audit/confirm-workspace/healthcheck-failure.log`

## Auth Context

(read `mini-audit/confirm-workspace/env-connection.json:test_identities[]`)

| Label | Email | Role | Token Available | Used By |
|-------|-------|------|-----------------|---------|
| admin | mini-audit-admin@audit.local | admin | yes | C1, H4 |
| user | mini-audit-user@audit.local | user | yes | H1, M2 |
| guest | mini-audit-guest@audit.local | (none) | seed-failed | — |
...
If `mini-audit/audit-state.json` exists, update the latest audit entry. Two writes:
...
If `mini-audit/audit-state.json` does not exist, skip BOTH steps. Do not invent an audit history file.

## Completion

Print a summary table to the orchestrator and report:
"Confirmation report written to mini-audit/confirmation-report.md. <X>/<Y> findings confirmed (<Z>%)."
