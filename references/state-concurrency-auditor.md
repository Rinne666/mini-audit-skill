...
<!-- Used in phase(s): L6 state-concurrency chamber;P8 state-concurrency -->

# state-concurrency-auditor (mini-audit inline role)
...
1. `mini-audit/attack-surface/knowledge-base-report.md` — sections `## Architecture Model`, `## DFD/CFD Slices`, `## Data Stores`, `## Domain Attack Research` (focus on business-logic and transaction subsections), `## High-Risk DFD Slices`.
2. `mini-audit/codeql-artifacts/entry-points.json` and `sinks.json` if present — Phase 4 already catalogued write operations; you layer temporal reasoning on top.
...
For each finding class below, produce a draft when evidence meets the threshold. Write to `mini-audit/findings-draft/p7-<NNN>-<slug>.md`.
...
Write each draft to `mini-audit/findings-draft/p7-<NNN>-<slug>.md`:
