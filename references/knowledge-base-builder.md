...
<!-- Used in phase(s): K1/K2 attack-surface KB build -->

# knowledge-base-builder (mini-audit inline role)
...
Before starting any discovery work, check whether `mini-audit/KNOWLEDGE-BASE.md` exists in the target repository (falling back to the legacy `mini-audit/INFO.md` if only that is present). If it does, read it first.

`mini-audit/KNOWLEDGE-BASE.md` is a hand-curated, project-specific context file (typically 50-100 lines) checked into the repo by maintainers. When present, it is **authoritative** for the items it covers — you must NOT re-derive them from the codebase.
...
| `## Known false-positive sources` | Add an explicit `## Known False-Positive Sources` section to `mini-audit/attack-surface/knowledge-base-report.md` reproducing each entry verbatim. Subsequent phases (Static Analyzer, Cold Verifier, Chamber agents) will skip findings that match these patterns. |
...
The orchestrator surfaces curated-context presence through the `MINI_AUDIT_KNOWLEDGE_BASE_AVAILABLE` environment variable (`true`/`false`); you may also check the file directly with `Read mini-audit/KNOWLEDGE-BASE.md` (or the legacy `Read mini-audit/INFO.md`).
...
- `mini-audit/attack-surface/knowledge-base-seed.md` — a cited, distilled seed (preferred when present).
- `mini-audit/attack-surface/knowledge-base-input/corpus.md` — the raw staged corpus (read this if no seed exists).
...
`~/.config/mini-audit/skills/audit/references/domain-attack-playbooks.md` before starting this step.
...
**Skip condition (incremental audits)**: skip domain attack research if the `## Domain Attack Research` section already exists in `mini-audit/attack-surface/knowledge-base-report.md`, no new relevant dependencies or specs were added since `audits[-1].commit`, and project type classification has not changed.
...
Produce a single `mini-audit/attack-surface/knowledge-base-report.md` containing all Phase 3 sections:
...
All Phase 3 KB content lives inside `mini-audit/attack-surface/knowledge-base-report.md` as sections. The one separate artifact is `mini-audit/attack-surface/unauthenticated-surface.md` (Step 6) — always write it.
