...
<!-- Used in phase(s): L5 cross-service audit;P8 cross-service chamber -->

# cross-service-auditor (mini-audit inline role)
...
If none of the heuristics fire, write `## Cross-Service Taint Propagation\n\nSkipped — single-service project; no inter-service edges detected.` to `mini-audit/attack-surface/knowledge-base-report.md` and exit cleanly. A no-op run is a legitimate outcome.

## Context Loading

Read, in order:

1. `mini-audit/attack-surface/knowledge-base-report.md` — `## Architecture Model`, `## DFD/CFD Slices`, `## Attack Surface`, `## High-Risk DFD Slices`
2. `mini-audit/probe-workspace/*/probe-summary.md` — every probe team's validated hypotheses per component. You will stitch these across components.
3. `mini-audit/codeql-artifacts/entry-points.json`, `sinks.json`, `call-graph-slices.json` if present (Phase 4 structural extraction)
4. `mini-audit/attack-surface/authz-matrix.md` if Phase 6 ran — it enumerates the endpoint surface you need to match producers against
...
For each call site, extract the URL string (literal or template). Match against endpoint paths discovered by authz-auditor (`mini-audit/attack-surface/authz-matrix.md`) or probe-workspace entry-point catalogues. Build edges: `serviceA:file:line → serviceB:handler`.
...
Write `mini-audit/attack-surface/cross-service-edges.json`:
...
Also write a human-readable summary to `mini-audit/attack-surface/cross-service-edges.md` listing each edge in a table.
...
Write drafts to `mini-audit/findings-draft/p8-<NNN>-<slug>.md`.
...
Append to `mini-audit/attack-surface/knowledge-base-report.md`:
...
- Coverage gaps: <unresolved templates / unmatched channels> — see `mini-audit/attack-surface/cross-service-edges.md`
