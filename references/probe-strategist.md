...
<!-- Used in phase(s): L5 deep probe strategy;P8 deep probe -->

# probe-strategist (mini-audit inline role)
...
- **KB path**: `mini-audit/attack-surface/knowledge-base-report.md`
- **Workspace**: `mini-audit/probe-workspace/<component>/`
...
Read `mini-audit/attack-surface/knowledge-base-report.md`: sections `## DFD/CFD Slices`, `## Attack Surface`, `## Architecture Model`, `## Domain Attack Research`.

**Read intent corpus** (revisit mode, optional): if `mini-audit/attack-surface/intent-corpus.json` exists, scan its `acknowledged_risks[]` array. The vuln classes listed there are ones the project explicitly says it cares about — treat them as a soft prioritization hint when picking which entry points to probe deepest. Do NOT skip entry points or classes that aren't on the list; the corpus is additive, not restrictive. If the corpus is missing or empty, proceed without it.

Then use Glob + Grep to find all source files for your assigned component(s).

Write `mini-audit/probe-workspace/<component>/attack-surface-map.md` with sections: Entry Points, Trust Boundary Crossings, Auth/AuthZ Decision Points, Validation/Sanitization Functions, Layer Trust Chain (table of layer transitions with trust assumptions and alternate paths), and Trust Chain Gaps.
...
`mini-audit/probe-workspace/<component>/code-anatomy.md`.
...
```
Attack surface map: mini-audit/probe-workspace/<component>/attack-surface-map.md
Code anatomy: mini-audit/probe-workspace/<component>/code-anatomy.md
Layer trust chain gaps: [paste the Trust Chain Gaps section]
Output file: mini-audit/probe-workspace/<component>/round-1-hypotheses.md
```
...
Code anatomy: mini-audit/probe-workspace/<component>/code-anatomy.md
Layer trust chain gaps: [paste the Trust Chain Gaps section]
Output file: mini-audit/probe-workspace/<component>/round-2-hypotheses.md
```
...
For each match, write a cross-model seed to `mini-audit/probe-workspace/<component>/cross-model-seeds.md`:
...
```
Hypotheses files:
  - mini-audit/probe-workspace/<component>/round-1-hypotheses.md
  - mini-audit/probe-workspace/<component>/round-2-hypotheses.md
Cross-model seeds: mini-audit/probe-workspace/<component>/cross-model-seeds.md
Component source paths: [from attack surface map]
Output file: mini-audit/probe-workspace/<component>/round-1-evidence.md
```
...
Needs-Deeper: <N>
Stop reason: <reason>
Summary: mini-audit/probe-workspace/<component>/probe-summary.md
```
