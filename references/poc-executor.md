...
<!-- Used in phase(s): L7 PoC execution;P13 PoC execution -->

# poc-executor (mini-audit inline role)
...
You receive:
- **Finding path**: `mini-audit/findings/<ID>-<slug>/`
- **Connection details**: `mini-audit/confirm-workspace/env-connection.json` OR a `--target` URL
- **Per-variant timeout**: default 30 seconds **per attempt** (max 2 attempts → 60s wall clock per finding)
- **Session UUID**: `$MINI_AUDIT_SESSION_UUID` (informational; used in evidence headers)
...
```bash
BASE_URL=$(jq -r '.base_url' mini-audit/confirm-workspace/env-connection.json)
if ! curl -sf -o /dev/null --max-time 5 "$BASE_URL"; then
  ...
  printf "Confirm-Status: blocked\nConfirm-Notes: app-unreachable-at-poc-start (%s)\nConfirm-Timestamp: %s\n" \
    "$BASE_URL" "$(date -u +%Y-%m-%dT%H:%M:%SZ)" >> mini-audit/findings/<ID>-<slug>/report.md
  exit 0
fi
```
...
Read the finding report at `mini-audit/findings/<ID>-<slug>/report.md`. Extract:
...
```
mini-audit/findings/<ID>-<slug>/poc.py
mini-audit/findings/<ID>-<slug>/poc.sh
mini-audit/findings/<ID>-<slug>/poc.js
mini-audit/findings/<ID>-<slug>/poc.rb
mini-audit/findings/<ID>-<slug>/poc.go
mini-audit/findings/<ID>-<slug>/exploit.sh
mini-audit/findings/<ID>-<slug>/exploit.py
```
...
Write the adapted script to `mini-audit/findings/<ID>-<slug>/confirm-evidence/poc-adapted.{ext}`.
...
```bash
mkdir -p mini-audit/findings/<ID>-<slug>/confirm-evidence/

cat > mini-audit/findings/<ID>-<slug>/confirm-evidence/env-info.txt <<EOF
Target: $BASE_URL
Timestamp: $(date -u +%Y-%m-%dT%H:%M:%SZ)
Method: $(jq -r '.method_used' mini-audit/confirm-workspace/env-connection.json)
...
restore_snapshot() {
  ...
  spec=mini-audit/confirm-workspace/snapshot-spec.json
  [ -f "$spec" ] || return 0
...
run_variant() {
  ...
  echo "--- variant ${variant_idx} @ $(date -u +%Y-%m-%dT%H:%M:%SZ) ---" \
    >> mini-audit/findings/<ID>-<slug>/confirm-evidence/attempts.log
  timeout --kill-after=5s 30s <interpreter> "$script" \
    2>&1 | tee -a mini-audit/findings/<ID>-<slug>/confirm-evidence/attempts.log
}

restore_snapshot
run_variant 1 mini-audit/findings/<ID>-<slug>/confirm-evidence/poc-adapted.{ext} \
  > mini-audit/findings/<ID>-<slug>/confirm-evidence/exploit.log
```
...
For **failed** results after both variants: run the `fp-check` skill on the original draft (`mini-audit/findings/<ID>-<slug>/draft.md`) using the live evidence as context. Two outcomes:
...
Record each attempt and the fp-check verdict in `mini-audit/findings/<ID>-<slug>/confirm-evidence/attempts.log`.
...
Confirm-Evidence: mini-audit/findings/<ID>-<slug>/confirm-evidence/
