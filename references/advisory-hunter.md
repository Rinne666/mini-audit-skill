...
<!-- Used in phase(s): L3 advisory CVE mining;P1 deep advisory;KB1 knowledge-base build -->

# advisory-hunter (mini-audit inline role)
...
OWNER_REPO="${MINI_AUDIT_REPOSITORY:-}"

# 2. Fall back to git remote if available
if [ -z "$OWNER_REPO" ] && [ "${MINI_AUDIT_GIT_AVAILABLE:-true}" = "true" ]; then
...
|-----------|------------------|------------------------|------------------------------|
| `MINI_AUDIT_GIT_AVAILABLE=true` AND `OWNER_REPO` is `owner/repo` | run | run | run locally via `git log/diff` |
| `MINI_AUDIT_GIT_AVAILABLE=false` AND `OWNER_REPO` is `owner/repo` | **skip** | run | run via `gh api repos/$OWNER/$REPO/compare/v1...v2` |
...
if [ "${MINI_AUDIT_GIT_AVAILABLE:-true}" = "true" ]; then
...
```bash
# OWNER and REPO were resolved in Step 0 (from MINI_AUDIT_REPOSITORY, git remote, or package
...
Write the full inventory to `mini-audit/attack-surface/sbom.json`:
...
Write the `## Advisory Intelligence` section of `mini-audit/attack-surface/knowledge-base-report.md` with:
...
- Repository identity: `<OWNER_REPO value>` (resolved via `<source: MINI_AUDIT_REPOSITORY env / git remote / package manifest <which> / basename fallback>`)
- Git history available: `true` / `false` (sourced from `MINI_AUDIT_GIT_AVAILABLE`)
...
Summarize the full component inventory (written in detail to `mini-audit/attack-surface/sbom.json`) as a `## Component Inventory` section in the KB:
...
Write `mini-audit/attack-surface/sbom.json` (Section 4c) as a standalone artifact. If `mini-audit/attack-surface/knowledge-base-report.md` does not yet exist, create it and add the section header. If it already exists, append or update the `## Advisory Intelligence` and `## Component Inventory` sections in-place.
