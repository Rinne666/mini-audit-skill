...
<!-- Used in phase(s): Q3 PoC construction;L6c PoC;P13 PoC;V3 confirm PoC -->

# poc-builder (mini-audit inline role)
...
You receive:
- **Finding draft path**: `mini-audit/findings-draft/<phase>-<NNN>-<slug>.md`
...
The orchestrator has already created `mini-audit/findings/<ID>-<slug>/` during draft promotion and populated it with:
...
Verify the directory exists. If missing, create it: `mkdir -p mini-audit/findings/<ID>-<slug>/evidence/`

### 3. Build the PoC Script

Write a minimized exploit script at `mini-audit/findings/<ID>-<slug>/poc.{py|sh|js}`.
...
Follow `~/.config/mini-audit/skills/audit/references/real-env-validation.md` for provisioning:
...
Evidence capture:
```bash
# Required files in mini-audit/findings/<ID>-<slug>/evidence/
...
Write back to the finding draft at `mini-audit/findings/<ID>-<slug>/draft.md`:
```
...
Do NOT write `mini-audit/findings/<ID>-<slug>/report.md`. Phase 14's finding-reporter owns that file — your job stops once the PoC, evidence, and draft metadata are in place.
