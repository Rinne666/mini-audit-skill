"""A.py — entry points for the report endpoint, v1.

Two callers reach the report query:

* ``handle_request`` is the HTTP route. It coerces the filter to a list of
  integers before use.
* ``run_import`` is the batch import path. **It also coerces the filter** in v1,
  so both callers pass a list of integers to the query.
...
def handle_request(request, db):
    ...
    absence of it (in a later revision) a discrepancy rather than a
...
def run_import(request, db):
    """Batch import path, v1 — also coerces the filter. No bypass exists yet."""
    filters = coerce_int_list(request.get("author_exclude"))
    return db.report(filters)
